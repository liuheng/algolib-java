...
public class Residue extends Applet implements Runnable {
	...
	public void set(int flag, int n, int m,
			int snode, int tnode, Node v[], _Edge e[])  {
		this.flag = flag;
		this.n = n;
		this.m = m;
		this.snode = snode;
	...
	void draw_Edge(Graphics g, FontMetrics fm,
	...
	public void paint_Edge(Graphics g, _Edge e, FontMetrics fm) {
		...
		if (e.capacity-e.flow==0)
			...
			g.setColor(c);
		draw_Edge(g,fm,v1.x+x1[0],v1.y+x1[1],v2.x+x2[0],v2.y+x2[1],
				e.capacity-e.flow);
		...
		if (e.flow==0)
			...
			g.setColor(c);
		draw_Edge(g,fm,v2.x+x1[0],v2.y+x1[1],v1.x+x2[0],v1.y+x2[1],
				e.flow);
	}
